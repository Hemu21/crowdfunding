...
const EditCampaign = () => {
  const { getCampaign, updateCampaign } = useWeb3Context();
  ...
  const getData = async () => {
    const data = await getCampaign(path?.id);
    setForm({
      title: data?.title,
      description: data?.description,
      target: data?.target,
      deadline: new Date(data.deadline * 1000).toISOString().slice(0, -1),
      image: data?.image,
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    await updateCampaign({
      ...form,
      target: EthToWei(form.target),
      deadline: convertToSolidityTimestamp(form.deadline),
      id: path?.id,
    });
    setForm({
      title: "",
